...
function loadSelectRoles(){
    var createSelect = function(data) {
        var html = "<select class='form-control' id='rol'>";
        data.forEach(element => {
            html += "<option value='"+element.id+"'>"+element.title+"</option>"
        });
        html += "</select>";
        document.getElementById('contenedor_rol').innerHTML = html;
    };
    getRoles(createSelect);
}
...
function clickFrmSubmit(e) {
    ...
    var slcRol = document.getElementById("rol");
    var rol = slcRol.value;
...
function printTable(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        html += "<td><img src='" + data[i].avatarFilePath + "' class='avatar' width='50px' height='50px' /></td>";
        html += "<td>" + data[i].rol.title + "</td>";
        html += "<td>";
...
function loadDataById(id) {
    ...
    xhr.onload = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            contenedor_avatar.classList.remove("hide");
            img_avatar.src = data.avatarFilePath;
...
function upload(fn){
    ...
    xhr.open("POST", "http://localhost:8082/file/upload", true);
        ...
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            if (fn !== null && fn !== undefined) {
                try {
                    ...
                    console.log("No se pudo ejecutar el callback para guardar info");
                }
            }
        } else {
            alert("Error al ejecutar la transaccion, el servidor no puede procesar la solicitud");
